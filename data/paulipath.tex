...
\begin{equation}\label{eq:spd_cost}
    C = C_s+N_MC_f,
\end{equation}
其中$C_s$是枚举所有具有非平凡贡献函数的Pauli路径的时间复杂度，$N_M$是满足条件的Pauli路径的数量，$C_f$是计算每个Pauli路径的贡献的时间复杂度可以通过式~\eqref{eq:cost_f}计算。

同样，类似于式~\eqref{eq:space_cost}，SPD算法的空间复杂度可以表示为：
\begin{equation}\label{eq:spd_space}
    S = S_s+S_f,
\end{equation}
其中$S_s$是枚举所有具有非平凡贡献函数的Pauli路径的空间复杂度，$S_f$是计算每个Pauli路径的贡献的空间复杂度可以通过式~\eqref{eq:space_f}计算。
...
\end{equation}
因此，SPD算法枚举过程的空间复杂度$S_s$与OBPPP的空间复杂度$S_s$相同，如式~\eqref{eq:path:space_cost}所示。
...
综上所述，对于SPD算法，枚举所有具有非平凡贡献函数的Pauli路径的时间复杂度为：
